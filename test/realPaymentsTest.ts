...
describe('Real Payments', () => {
  ...
  it('test add ', async () => {
    ...
    const balance = await dbClass.getDb().collection('balance').findOne({"payer.personalAccount": doc.from.personalAccount})
    expect(balance.value).eq(10850)
    const orgD = await dbClass.getDb().collection('report_dayli_organization').findOne({"organization.personalAccount": "11","stamp":"2016-4-31"})
    expect(orgD.value).eq(700)
    const orgM = await dbClass.getDb().collection('report_month_organization').findOne({"organization.personalAccount": "11","stamp":"2015-7"})
    expect(orgM.value).eq(700)
    const orgY = await dbClass.getDb().collection('report_year_organization').findOne({"organization.personalAccount": "11","stamp":"2016"})
    expect(orgY.value).eq(8650)
  })
  it('test undo', async () => {
    const s  = await dbClass.getDb().collection('payments').findOne({_id: paymentId})
    const orgD2 = await dbClass.getDb().collection('report_dayli_organization').findOne({"organization.personalAccount": "11","stamp":"2014-7-1"})
    expect(orgD2.value).eq(700.25)
    await undoPaymentHandler(dbClass, { _id: paymentId })
    const balance = await dbClass.getDb().collection('balance').findOne({"payer.personalAccount": s.from.personalAccount})
    expect(balance.value).eq(10500)
    const orgD = await dbClass.getDb().collection('report_dayli_organization').findOne({"organization.personalAccount": "11","stamp":"2014-7-1"})
    expect(orgD.value).eq(350.25)
    const orgM = await dbClass.getDb().collection('report_month_organization').findOne({"organization.personalAccount": "11","stamp":"2015-7"})
    expect(orgM.value).eq(700)
    const orgY = await dbClass.getDb().collection('report_year_organization').findOne({"organization.personalAccount": "11","stamp":"2016"})
    expect(orgY.value).eq(8650)
